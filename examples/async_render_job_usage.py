"""Render job example (asynchronous).

This example demonstrates how to use the async client to:
1. Find a completed text2motion job
2. Start a render job (animation to video)
3. Receive progress updates via callback
4. Download results
"""
    ...
    async with AsyncSaymotionClient(
        ...
        async def on_result(data: ResultCallbackData):
            ...
            if data.result:
                ...
                print(
                    f"Render job failed: {data.error.message} "
        ...
        if not jobs:
            ...
            print("Run async_basic_usage.py first to create a text2motion job.")
            return
